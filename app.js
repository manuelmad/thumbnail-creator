(function () {
  ...
  function selectPlatform(selectedBtn) {
    ...
    if (profilesDiv) {
      if (selectedBtn === btnTiktok) {
        profilesDiv.innerHTML = `
          <div style="display:flex; flex-direction:row; gap:4px; font-size:12px; margin-top:8px; justify-content:center; align-items:center;">
            <label><input type="radio" name="profile" value="atari"> Atari 2600</label>
            <label><input type="radio" name="profile" value="club"> Club Nintendo</label>
            <label><input type="radio" name="profile" value="game"> Game & Watch</label>
            <label><input type="radio" name="profile" value="mortal"> Mortal Kombat</label>
          </div>
        `;
      } else {
        profilesDiv.innerHTML = '';
  ...
  if (profilesDiv) {
    profilesDiv.addEventListener('change', (e) => {
  ...
  function applyProfile(fontSize, titleY, overlayScale, overlayY) {
    if (fontSizeRange) fontSizeRange.value = fontSize;
    if (fontSizeNumber) fontSizeNumber.value = fontSize;
    if (titlePreview) titlePreview.style.fontSize = fontSize + 'px';

    if (titleYRange) titleYRange.value = titleY;
    if (titleYNumber) titleYNumber.value = titleY;

    if (overlaySizeRange) overlaySizeRange.value = overlayScale;
    if (overlaySizeNumber) overlaySizeNumber.value = overlayScale;
    if (overlayElement) overlayElement.style.width = overlayScale + '%';

    if (overlayYRange) overlayYRange.value = overlayY;
    if (overlayYNumber) overlayYNumber.value = overlayY;
    if (overlayElement) overlayElement.style.top = overlayY + '%';

    if (typeof updateTitlePosition === 'function') updateTitlePosition();
  }
  if (btnTiktok) {
